# postgis/postgis - PostGIS Docker Images
...
**⚠️ Rolling `*-master` Builds (effective 2026-06-22):**
`*-master` builds use freshly resolved upstream HEADs. Published images are refreshed on the CI schedule and after builds of the repository's master branch. Do not rely on the checked-in Dockerfiles to identify image contents; inspect the image's `*_GIT_HASH` environment variables instead. Other tags are unaffected. See the [Test images](#test-images) section for details.

## Image Overview

The `postgis/postgis` image provides tags for running Postgres with [PostGIS](http://postgis.net/) extensions installed. This image is based on the official [`postgres`](https://registry.hub.docker.com/_/postgres/) image and provides Debian and Alpine variants for PostGIS 3.5.x (PostgreSQL 14 through 17) and PostGIS 3.6.x (PostgreSQL 17 through 18), as well as PostgreSQL 19 pre-release versions. Additionally, image variants are provided for PostgreSQL 16 through 18 and PostgreSQL 19 pre-release builds, built with PostGIS and its dependencies from their upstream development branches. These include `16-master`, `17-master`, `18-master`, and a PostgreSQL 19 pre-release `*-master` image.
...
* Current PostGIS 3.7 development images use the up-to-date CGAL revision from the `6.2.x-branch`.
...
**Warning for `*-master` images (effective 2026-06-22):**

`*-master` Dockerfiles are regenerated with the latest upstream Git commit hashes immediately before each build. The checked-in Dockerfiles may therefore differ from the published images. Inspect the image metadata to identify the exact commits used:

```
docker image inspect --format '{{range .Config.Env}}{{println .}}{{end}}' postgis/postgis:18-master | grep '_GIT_HASH='
```

The same commit details are also stored inside the image in `/_pgis_*_last_commit.txt` files.
...
Every `postgis/postgis:18-*` and newer image includes `io_uring` capabilities for asynchronous I/O.  However, some container runtimes (for example, [containerd](https://github.com/containerd/containerd/issues/9048)) have **disabled `io_uring` support** in the past due to **security concerns**.
